...
export const Tool = ({ className, ...props }: ToolProps) => (
  <Collapsible
    className={cn(
      "group not-prose mb-2 w-full rounded-md border border-white/5",
      className
    )}
    {...props}
  />
...
export const getStatusBadge = (status: ToolPart["state"]) => (
  <Badge
    className="gap-1 rounded-full text-[10px] opacity-60"
    variant="secondary"
  >
    {statusIcons[status]}
    {statusLabels[status]}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <WrenchIcon className="size-3.5 text-muted-foreground/50" />
